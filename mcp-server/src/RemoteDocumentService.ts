...
export class RemoteDocumentService {
  ...
  async listDocuments(category: 'frontend' | 'backend' | 'all' = 'all'): Promise<Document[]> {
    ...
    if (cached && Date.now() - (cached as any).timestamp < this.documentCacheTimeout) {
      return cached;
    }
    ...
    // 缓存结果
    this.documentCache.set(cacheKey, filteredDocuments as any);
    (this.documentCache.get(cacheKey) as any).timestamp = Date.now();
   ...
   * 动态发现所有文档
   */
  private async discoverDocuments(): Promise<Document[]> {
   ...
   * 通过爬取页面链接发现文档
   */
  private async discoverFromCrawling(): Promise<Document[]> {
    try {
   ...
   * 基于常见路径模式发现文档
   */
  private async discoverFromCommonPatterns(): Promise<Document[]> {
    const commonPaths = [
      '/',
   ...
   * 发现组件文档路径
   */
  private async discoverComponentPaths(): Promise<string[]> {
    ...
    for (const indexPath of indexPaths) {
      try {
        ...
        if (content) {
          const links = this.extractLinksFromHtml(content.content);
          componentPaths.push(...links.filter(link => 
            link.includes('/frontend/components/') && 
            link !== indexPath
          ));
        }
      } catch (error) {
   ...
   * 获取基本文档列表（降级方案）
   */
  private getBasicDocuments(): Document[] {
    return [
