...
export class SearchService {
    ...
    async search(query, options = {}) {
        ...
        const { category = 'all', limit = 10, threshold = 0.4, } = options;
        // 设置搜索阈值
        ...
        // 应用阈值过滤
        filteredResults = filteredResults.filter(result => result.score !== undefined && result.score <= threshold);
        // 限制结果数量
    ...
    generateExcerpt(text, matches, query) {
        // 移除HTML标签
    ...
    // 高级搜索：支持多个查询词
    async advancedSearch(queries, options = {}) {
        const allResults = [];
    ...
    // 获取相关文档
    async getRelatedDocuments(path, limit = 5) {
